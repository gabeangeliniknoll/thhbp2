...
\begin{remark}
  ...
  % This is also consistent with~\cite[Theorem~3.5]{AR20}, which we believe can be adapted to $\mathrm{BP}\langle n\rangle[v_{n+1}^{-1}]$ for an arbitrary $\mathbb{E}_3$-$\mathrm{MU}$-algebra form along the lines of~\cite[\S~5.1]{ACH24}.

  % \maxime{i need to check that last claim above}
  %%%%% It isn't clear for me what kind of conjecture can be formulated for the torsion free part, so that I don't know how it would compare to the E(2) result
