...
%The homology of topological Hochschild homology of $\B$ is a straightforward application of work Steinberger \cite{BMMS86}, B\"okstedt \cite{Bok85}, and Angeltveit-Rognes \cite{AngeltveitRognes}, and it appears in  \cite[Thm. 5.12]{AngeltveitRognes}. 
Recall that at a prime $p \ge 3$ there is an isomorphism
\[H_*(\B)\cong %\begin{cases} 
P(\bxi_1,\bxi_2,\ldots )\otimes E(\btau_{3},\btau_4,\ldots) \subset \A_*   %\\ P(\bxi_1^2,\bxi_2^2,\bxi_3^2,\bxi_4,\ldots) \text{ if } p=2 
%\end{cases}
\]
of $\A_*$-comodules. By \cite[Thm. 5.12]{AngeltveitRognes}, there is an isomorphism
\begin{equation}\label{HTHHB} H_*(\THH(\B))\cong %\begin{cases} 
H_*\B \otimes E(\sig{\bxi_1},\sig{\bxi_2},\sig{\bxi_3})\otimes P(\sig{\btau_3}) %\\ H_*\B\otimes E(\sig{\bxi^2_1},\sig{\bxi^2_2},\sig{\bxi^2_3})\otimes P(\sig{\bxi_4}) \text{ if }p=2
%\end{cases}
\end{equation}
...
By \eqref{HTHHB}, we know that $H_*THH(\B)$ is free over $H_*(\B)$. Since $\B$ is a commutative ring spectrum, there is a splitting 
\[THH(\B;M)\sim \B\vee \overline{THH}(\B;M)\] 
for any $\B$-module and consequently the input becomes 
...
\gabe{Adjust this based on other paper.}
We now compute 
\[\THH_*(\B;H\F_p).\] 
...
H_*(\THH(\B;H\F_p))\cong %\begin{cases} 
\A_* \otimes E(\sigma \bxi_1, \sigma \bxi_2, \sigma \bxi_3)\otimes P(\sigma \btau_3) %\text{ if } p=3 
%\\  \A_* \otimes E(\sigma \bxi_1^2, \sigma \bxi_2^2, \sigma \bxi_3^2)\otimes P(\sigma \bxi_3) \text{ if } p=2. \end{cases}
\end{equation}
...
We write $\lambda_i=\sigma\bxi_i$ and define $\mu_3:= \sigma \btau_3 -\btau_0\sigma \bxi_3$ %at $p=3$ and $\lambda_i=\sigma \bxi_i^2$ are $p=2$. We also define 
%\[ \mu_3=\begin{cases} \sigma \btau_3 -\btau_0 \sigma \bxi_3 \text{ if } p=3 \\ \sigma \bxi_4-\bxi_1 \sigma \bxi^2_3 \text{ if } p=2 \end{cases}. \]
...
\]
where $|\sigma v_i|=2p^i-1$.  We will suggestively rename $\sigma v_i=\lambda_i$, which we will validate later. Consequently, 
\begin{equation}\label{Qcoeff} THH_*(\B;H\Q)\cong E(\lambda_1,\lambda_2) \end{equation}.
